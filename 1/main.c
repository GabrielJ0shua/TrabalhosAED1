...
int main(){
    ...
    while(scanf("%d", &op) && op != 11){
        ...
        if(ini == 0 && op != 1){ // NAO DEIXA FAZER ALGO COM A LISTA SE ELA NAO EXISTIR
            printf("\n### Lista NAO Inicializada ###\n\n");
            menu();
            continue;
        }

        if(ini == 1 && op == 1){ // IMPEDIRA QUE APAGUE A LISTA USANDO O CRIA LISTA, JA QUE TEM O COMANDO DE ESVAZIAR
